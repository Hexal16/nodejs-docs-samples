...
require(`yargs`) // eslint-disable-line
  .demand(1)
  .command(
    `sync <sqlQuery> [projectId]`,
    `Run the specified synchronous query.`,
    {},
    (opts) => syncQuery(opts.sqlQuery, opts.projectId || process.env.GCLOUD_PROJECT)
  )
  .command(
    `async <sqlQuery> [projectId]`,
    `Start the specified asynchronous query.`,
    {},
    (opts) => asyncQuery(opts.sqlQuery, opts.projectId || process.env.GCLOUD_PROJECT)
  )
  .command(
    `shakespeare`,
    `Queries a public Shakespeare dataset.`,
    {},
    (opts) => queryShakespeare(opts.projectId || process.env.GCLOUD_PROJECT)
  )
  .example(
